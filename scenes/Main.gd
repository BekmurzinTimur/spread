...
func _drain_events() -> void:
	...
	for cell_id in world.take_mine_events():
		...
		if cell.is_boss:
			_splash_layer.spawn(cell.position, Color(1.0, 0.97, 0.85), 3.0)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT and not event.pressed:
			# A release that ended a pan is not an aim.
			if ram_armed and not _camera.panned:
...
## A hex click. Power arms the ram; the rest activate in the sim.
func activate_skill(id: String) -> void:
	if world == null or phase != Phase.RUNNING:
		return
